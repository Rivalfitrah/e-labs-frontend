  ...
  <div class="bg-[#F6FBF6] min-h-screen">
    ...
    <div class="container mx-auto p-8">
      <div class="text-center mb-10">
        <h1 class="text-3xl font-bold text-gray-800">Peminjaman Ruangan Lab Komputer di Luar Jam Kuliah</h1>
        <p class="text-gray-600 mt-2">Anda harus menyiapkan surat peminjaman secara digital yang sudah ditandatangani</p>
      </div>
        ...
        <table class="w-full text-sm text-left">
          ...
          <tbody class="divide-y divide-gray-200">
            <tr v-for="lab in labs" :key="lab.id" class="hover:bg-gray-50">
              <td class="px-6 py-4 font-semibold text-gray-800">{{ lab.name }}</td>
              <td class="px-6 py-4 text-gray-600">{{ lab.location }}</td>
              <td class="px-6 py-4 text-center text-gray-600">{{ lab.capacity }}</td>
              <td class="px-6 py-4 text-center">
              <span
                :class="[
                    'px-3 py-1 text-xs font-bold rounded-full',
                    lab.status === 'available' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                ]"
                >
              ...
              <td class="px-6 py-4 text-center">
                <NuxtLink v-if="lab.status === 'available'" to="/form-peminjaman" class="text-green-600 font-semibold hover:underline">Pinjam Ruangan</NuxtLink>
                <span v-else class="text-gray-500">Terpinjam</span>
              </td>
              <td class="px-6 py-4 text-center">
                <button v-if="lab.status === 'booked'" @click="openDetailModal(lab)" class="text-blue-600 hover:underline">
                  Lihat Detail
                </button>
              </td>
            </tr>

<script setup>
  ...
  onMounted(async () => {
    try {
      ...
      const actualData = Array.isArray(roomsArray)
        ? roomsArray
        : roomsArray?.data || [];
      ...
      try {
        ...
        const tersedia = actualData.filter(
          (r) => r.status_ruangan?.toLowerCase() === "tersedia"
        ).length;
        const dipinjam = actualData.filter(
          (r) => r.status_ruangan?.toLowerCase() === "dipinjam"
        ).length;
        const tidakTersedia = actualData.filter(
          (r) =>
            r.status_ruangan?.toLowerCase() === "tidak tersedia" ||
            r.status_ruangan?.toLowerCase() === "tidak_tersedia"
        ).length;
  ...
  async function handleSave() {
    ...
    try {
      ...
      // Update local list
      const index = ruangan.value.findIndex(
        (r) => r.id === selectedRuangan.value.id
      );
      if (index !== -1) {
  ...
  async function handleAddSubmit() {
    if (
      !newRuangan.value.nama_ruangan ||
      !newRuangan.value.kode_ruangan ||
      !newRuangan.value.gedung
    ) {
    ...
    try {
      ...
      showNotification(
        "Gagal menambah ruangan. Cek konsol dan pastikan Kode Ruangan unik.",
        "error"
      );
    }
  }
...
<template>
  ...
  <div v-else class="shadow-xl rounded-xl overflow-hidden bg-white">
    ...
    <div class="hidden md:block">
      <table class="w-full border-separate border-spacing-0">
        ...
        <tbody class="text-gray-700 text-sm text-center">
          <tr
            v-for="(data, index) in paginatedRuangan"
            :key="data.kode_ruangan"
          ...
          >
            ...
            <td class="px-5 py-3 font-semibold align-middle">
              <span
                class="bg-gray-100 text-gray-700 px-2 py-0.5 rounded-full text-xs font-medium whitespace-nowrap"
              >
                {{ data.gedung }}
              </span>
            </td>
            ...
            <td class="px-5 py-3 align-middle">
              ...
              >
                <img
                  ...
                  src="https://placehold.co/40x40/f1f1f1/333333?text=..."
                  alt="Loading QR"
                ...
                <img
                  ...
                  :src="
                    data.QR_Image_url
                      ? `${storage_URL}/uploads/${data.QR_Image_url}`
                      : 'https://placehold.co/40x40/f1f1f1/333333?text=QR'
                  "
                  alt="QR Code Ruangan"
                  class="w-full h-full object-cover"
                  @error="
            ...
            <td class="px-5 py-3 text-center align-middle">
              <div class="flex justify-center items-center gap-2">
                ...
                <!-- Tombol Download QR -->
                <a
                  v-if="data.QR_Image_url"
                  href="javascript:void(0)"
                  @click.stop="
                    downloadQR(
                      `${storage_URL}/uploads/${data.QR_Image_url}`,
                      `QR_${data.gedung}_Ruangan_${data.kode_ruangan}.png`
                    )
                  "
          ...
          <tr
            v-if="paginatedRuangan.length === 0 && filteredRuangan.length > 0"
          >
    ...
    <div class="md:hidden divide-y divide-gray-200">
        ...
        <div class="flex items-start gap-3">
          ...
          <div class="flex shrink-0">
            ...
            >
              <img
                v-if="qrLoadingIds.includes(data.id)"
                src="https://placehold.co/64x64/f1f1f1/333333?text=..."
                alt="Loading QR"
                class="w-full h-full object-cover"
              />
              <img
                ...
                :src="
                  data.QR_Image_url
                    ? `${storage_URL}/uploads/${data.QR_Image_url}`
                    : 'https://placehold.co/64x64/f1f1f1/333333?text=QR'
                "
                alt="QR Code Ruangan"
                class="w-full h-full object-cover"
                @error="
      ...
      <div
        v-else-if="filteredRuangan.length === 0"
        class="p-8 text-center text-gray-500 text-sm"
      >
        Tidak ada hasil yang cocok dengan pencarian Anda.
      </div>
  ...
  >
    ...
    >
      <h2 class="text-2xl font-extrabold text-gray-800 mb-4 border-b pb-2">
        Ubah Data Ruangan:
        <span class="text-primary">{{ selectedRuangan?.nama_ruangan }}</span>
      </h2>
      ...
      >
        ...
        <div>
          <label
            for="modal_kode_ruangan"
            class="block text-sm font-medium text-gray-700 mb-1"
            >Kode Ruangan</label
          >
          <input
            id="modal_kode_ruangan"
            type="text"
            :value="selectedRuangan.kode_ruangan"
        ...
        <div>
          <label
            for="modal_nama_ruangan"
            class="block text-sm font-medium text-gray-700 mb-1"
            >Nama Ruangan</label
          >
          <input
            id="modal_nama_ruangan"
            type="text"
            v-model="selectedRuangan.nama_ruangan"
        ...
        <div>
          ...
          <input
            id="modal_gedung"
            type="text"
            v-model="selectedRuangan.gedung"
            class="w-full border border-gray-300 rounded-lg p-2 focus:ring-primary focus:border-primary transition"
            required
          />
        </div>
  ...
  >
    ...
    >
      ...
      <form @submit.prevent="handleAddSubmit" class="space-y-4">
        ...
        <div>
          <label
            for="add_kode_ruangan"
            class="block text-sm font-medium text-gray-700 mb-1"
            >Kode Ruangan (ID Unik)</label
          >
          <input
            id="add_kode_ruangan"
            type="text"
            v-model="newRuangan.kode_ruangan"
            class="w-full border border-gray-300 rounded-lg p-2 focus:ring-primary focus:border-primary transition"
            required
          />
        </div>
        ...
        <div>
          ...
          <input
            id="add_nama_ruangan"
            type="text"
            v-model="newRuangan.nama_ruangan"
            class="w-full border border-gray-300 rounded-lg p-2 focus:ring-primary focus:border-primary transition"
            required
          />
        </div>

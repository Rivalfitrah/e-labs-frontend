<script setup>
  ...
  async function handleSave() {
    ...
    try {
      ...
      // Update local list
      const index = barang.value.findIndex(
        (b) => b.id === selectedBarang.value.id
      );
      if (index !== -1) {
      ...
      showNotification(
        "Gagal menyimpan perubahan data. Periksa konsol.",
        "error"
      );
      return;
    }
  ...
  async function handleImageUpload() {
    ...
    try {
      ...
      const newFotoUrl =
        response?.data?.foto_barang_url || imageFile.value.name;
      ...
      if (error.response && error.response.status === 401) {
        showNotification(
          "Otentikasi Gagal (401). Periksa konfigurasi CORS di server Anda!",
          "error"
        );
      } else {
        showNotification(
          "Gagal mengunggah gambar. Coba cek Network tab.",
          "error"
        );
      }
    }
  ...
  async function handleAddSubmit() {
    ...
    try {
      ...
      showNotification(
        "Gagal menambah barang. Cek konsol dan pastikan Kode Barang unik.",
        "error"
      );
    }
  }
  ...
  const imagePreviewUrl = computed(() => {
    ...
    return selectedBarang.value.foto_barang_url
      ? `${storage_URL}/${selectedBarang.value.foto_barang_url}`
      : "https://placehold.co/150x150/f1f1f1/333333?text=N/A";
  });
  ...
  <div>
    ...
    <div v-else class="shadow-xl rounded-xl overflow-hidden bg-white">
      ...
      <div class="hidden md:block overflow-x-auto">
        <table
          class="w-full table-auto border-separate border-spacing-0 min-w-max"
        >
          <thead>
            <tr
              class="bg-primary text-white text-center text-xs uppercase tracking-wider"
            >
          ...
          <tbody class="text-gray-700 text-xs text-center">
            <tr
              v-for="(data, index) in paginatedBarang"
              :key="data.kode_barang"
            ...
            >
              ...
              <td class="px-2 py-3 align-middle">
                ...
                >
                  <img
                    :src="
                      data.foto_barang_url
                        ? `${storage_URL}/${data.foto_barang_url}`
                        : 'https://placehold.co/32x32/f1f1f1/333333?text=N/A'
                    "
                    alt="Foto Barang"
              ...
              <td class="px-3 py-3 font-semibold align-middle">
                <div class="max-w-[120px] truncate" :title="data.nama_barang">
                  {{ data.nama_barang }}
                </div>
              </td>
              <td class="px-2 py-3 font-mono text-gray-500 align-middle">
                <div class="max-w-[80px] truncate" :title="data.kode_barang">
                  {{ data.kode_barang }}
                </div>
              </td>
              <td class="px-2 py-3 font-mono text-gray-500 align-middle">
                <div
                  ...
                  :title="data.kategori.nama_kategori"
                >
                  {{ data.kategori.nama_kategori }}
                </div>
              </td>
              <td class="px-2 py-3 align-middle">
                <div class="max-w-[80px] truncate" :title="data.merek">
                  {{ data.merek }}
                </div>
              </td>
              <td class="px-2 py-3 align-middle">
                <span
                  :class="{
                    'bg-green-100 text-green-800':
                      data.kondisi.toLowerCase() === 'baik',
                    'bg-yellow-100 text-yellow-800':
                      data.kondisi.toLowerCase() === 'rusak ringan',
                    'bg-red-100 text-red-800':
                      data.kondisi.toLowerCase() === 'rusak berat',
                  }"
              ...
              <td class="px-2 py-3 font-mono align-middle">
                <div class="max-w-[80px] truncate" :title="data.status">
                  {{ data.status }}
                </div>
              </td>
            ...
            <tr
              v-if="paginatedBarang.length === 0 && filteredBarang.length > 0"
            >
      ...
      <div class="md:hidden divide-y divide-gray-200">
              ...
              >
                <img
                  :src="
                    data.foto_barang_url
                      ? `${storage_URL}/${data.foto_barang_url}`
                      : 'https://placehold.co/64x64/f1f1f1/333333?text=N/A'
                  "
                  alt="Foto Barang"
                  class="w-full h-full object-cover"
        ...
        <div
          v-else-if="filteredBarang.length === 0"
          class="p-8 text-center text-gray-500 text-sm"
        >
          Tidak ada hasil yang cocok dengan pencarian Anda.
        </div>
      ...
      >
        <span class="text-sm text-gray-700">
          ...
          sampai
          <span class="font-semibold">{{
            Math.min(currentPage * itemsPerPage, filteredBarang.length)
          }}</span>
          dari
        ...
        <nav class="flex items-center space-x-1" aria-label="Pagination">
          ...
          <div class="flex space-x-1">
            <template v-for="(page, idx) in paginationPages" :key="idx">
              ...
              <span v-else class="px-3 py-2 text-gray-400 select-none text-sm"
                >...</span
              >
            </template>
          </div>
    ...
    >
      ...
      <!-- Modal Content -->
      <div
        class="bg-white rounded-xl shadow-2xl w-full max-w-lg p-4 sm:p-6 relative transition-all duration-300 my-8"
        @click.stop
      >
        <!-- Header -->
        ...
        >
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label
                for="modal_nama_barang"
                class="block text-sm font-medium text-gray-700 mb-1"
                >Nama Barang</label
              >
              <input
                id="modal_nama_barang"
                type="text"
            ...
            <div>
              <label
                for="modal_kode_barang"
                class="block text-sm font-medium text-gray-700 mb-1"
                >Kode Barang</label
              >
              <input
                id="modal_kode_barang"
                type="text"
            ...
            <div>
              ...
              <select
                id="modal_kondisi"
                v-model="selectedBarang.kondisi"
                class="w-full border border-gray-300 rounded-lg p-2 focus:ring-primary focus:border-primary transition bg-white"
                required
              >
                <option
                ...
                <option
                  v-if="
                    !['BAIK', 'RUSAK_RINGAN', 'RUSAK_BERAT', 'HILANG'].includes(
                      selectedBarang.kondisi
                    )
                  "
                  :value="selectedBarang.kondisi"
                  selected
                  hidden
                >
                  {{ selectedBarang.kondisi }}
                ...
                <option
                  v-if="
                    ![
                      ...
                      'PERBAIKAN',
                      'TIDAK_TERSEDIA',
                    ].includes(selectedBarang.status)
                  "
          ...
          <div>
            ...
            <input
              id="modal_jumlah"
              type="number"
              v-model.number="selectedBarang.jumlah"
              class="w-full border border-gray-300 rounded-lg p-2 focus:ring-primary focus:border-primary transition"
              min="1"
              required
            />
    ...
    >
      ...
      >
        <h2
          class="text-lg sm:text-2xl font-extrabold text-gray-800 mb-4 border-b pb-2"
        >
      ...
      >
        ...
        <form @submit.prevent="handleAddSubmit" class="space-y-4">
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label
                for="add_kategori_barang"
                class="block text-sm font-medium text-gray-700 mb-1"
                >Kategori Barang</label
              >
              <select
                id="add_kategori_barang"
                v-model="newBarang.kategori_id"
                class="w-full border border-gray-300 rounded-lg p-2 focus:ring-primary focus:border-primary transition bg-white"
                required
              >
          ...
          <div>
            ...
            <input
              id="add_jumlah"
              type="number"
              v-model.number="newBarang.jumlah"
              class="w-full border border-gray-300 rounded-lg p-2 focus:ring-primary focus:border-primary transition"
              min="1"
              required
            />

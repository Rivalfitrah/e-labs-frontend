...
const newBarang = ref({
    ...
    jumlah: 1,
    status: 'Tersedia',
    // Tambahkan field lain yang mungkin diperlukan (misal: kategori_id)
})
...
<template>
  ...
  <div v-else class="shadow-xl rounded-xl overflow-hidden bg-white">
    ...
    <table class="w-full border-separate border-spacing-0">
      ...
      <tbody class="text-gray-700 text-sm text-center">
        ...
        <tr v-for="(data, index) in paginatedBarang" :key="data.kode_barang" class="hover:bg-gray-50 transition border-t border-gray-100">
          <td class="px-5 py-3 font-mono align-middle">{{ (currentPage - 1) * itemsPerPage + index + 1 }}</td>
          <td class="px-5 py-3 align-middle">
            <div class="w-10 h-10 rounded-full overflow-hidden border border-gray-300 flex-shrink-0 mx-auto flex items-center justify-center">
              <img :src="data.foto_barang_url ? `${storage_URL}/${data.foto_barang_url}` : 'https://placehold.co/40x40/f1f1f1/333333?text=N/A'"
                   alt="Foto Barang"
  ...
  <div v-if="isEditModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 transition-opacity duration-300" @click.self="closeEditModal">
    ...
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg p-6 relative transform transition-all duration-300 scale-100 opacity-100"
      ...
      <h2 class="text-2xl font-extrabold text-gray-800 mb-4 border-b pb-2">
        Ubah Data Barang: <span class="text-primary">{{ selectedBarang?.nama_barang }}</span>
      </h2>
      ...
      <form v-if="selectedBarang" @submit.prevent="handleSave" class="space-y-4">
        ...
        <div>
          <label for="modal_kondisi" class="block text-sm font-medium text-gray-700 mb-1">Kondisi</label>
            ...
            <option
              v-if="!['BAIK','RUSAK_RINGAN','RUSAK_BERAT','HILANG'].includes(selectedBarang.kondisi)"
  ...
  <div v-if="isImageModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 transition-opacity duration-300" @click.self="closeImageModal">
    <!-- Overlay -->
    ...
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg p-6 relative transform transition-all duration-300 scale-100 opacity-100"
      ...
      <form @submit.prevent="handleImageUpload" class="space-y-4">
        ...
        <div v-if="imageFile || selectedBarang.foto_barang_url" class="mt-4 flex flex-col items-center">
          ...
          />
          <p v-if="imageFile" class="mt-2 text-sm text-gray-600">{{ imageFile.name }} ({{ (imageFile.size / 1024 / 1024).toFixed(2) }} MB)</p>
  ...
  <div v-if="isAddModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 transition-opacity duration-300" @click.self="closeAddModal">
    ...
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg p-6 relative transform transition-all duration-300 scale-100 opacity-100"
      ...
      <form @submit.prevent="handleAddSubmit" class="space-y-4">
        ...
        <!-- Kode Barang -->
        <div>
          <label for="add_kode_barang" class="block text-sm font-medium text-gray-700 mb-1">Kode Barang (ID Unik)</label>
          <input
            id="add_kode_barang"
            type="text"
